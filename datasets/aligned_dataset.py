...
def get_transform(params, resize_size, crop_size, method=Image.BICUBIC, flip=True, crop=True, totensor=True):
    ...
    transform_list.append(transforms.Lambda(lambda t: (t * 2) - 1))  # [0,1] --> [-1, 1])
...
def __scale(img, target_width, method=Image.BICUBIC):
    if isinstance(img, torch.Tensor):
        return torch.nn.functional.interpolate(
            img.unsqueeze(0), size=(target_width, target_width), mode="bicubic", align_corners=False
        ).squeeze(0)
    else:
...
class EdgesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        transform_image = get_transform(
            params, self.resize_size, self.crop_size, crop=self.random_crop, flip=self.random_flip
        )
...
class DIODE(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.filenames = [
            l
            for l in os.listdir(self.image_root)
            if not l.endswith(".pth") and not l.endswith("_depth.png") and not l.endswith("_normal.png")
        ]
    ...
    def __getitem__(self, index):
        ...
        transform_label = get_transform(
            params, self.resize_size, self.crop_size, method=Image.NEAREST, crop=False, flip=self.random_flip
        )
