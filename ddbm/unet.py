...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    # def forward(self, x, emb, use_fp16_or_bf16=False, dtype=th.bfloat16):
    #     for layer in self:
    #         if isinstance(layer, TimestepBlock):
    #             x = layer(x, emb)
    #         else:
    #             if isinstance(layer, QKVFlashAttention):
    #                 if use_fp16_or_bf16:
    #                     x = layer(x.to(dtype=dtype))
    #             else:
    #                 x = layer(x)
    #     return x
    def forward(self, x, emb, use_fp16_or_bf16=False, dtype=th.bfloat16):
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if use_fp16 or use_bf16:
            ...
            self.use_fp16_or_bf16 = True
        else:
            self.dtype = th.float32
            self.use_fp16_or_bf16 = False
        self.num_heads = num_heads
        self.num_head_channels = num_head_channels
        self.num_heads_upsample = num_heads_upsample
    ...
    def convert_to_fp16(self):
        ...
        if self.num_classes is not None:
            self.label_emb = self.label_emb.apply(convert_module_to_f16)
    ...
    def convert_to_bf16(self):
        ...
        self.out.apply(convert_module_to_bf16)
        if self.num_classes is not None:
            self.label_emb = self.label_emb.apply(convert_module_to_bf16)
    ...
    def convert_to_fp32(self):
        ...
        if self.num_classes is not None:
            self.label_emb = self.label_emb.apply(convert_module_to_f32)

    def forward(self, x, timestep, xT=None, y=None):
        """
        Apply the model to an input batch.
        ...
        # assert not th.isnan(emb).any(), f'timestep: {timestep}'
        ...
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            # print(y.max())
            # assert y.max().item() < self.num_classes, y.max()
            y_emb = self.label_emb(y.to(th.int32)).to(emb.dtype)
            emb = emb + y_emb

        h = x.type(self.dtype)
        for module in self.input_blocks:
            if self.use_fp16_or_bf16:
                if self.use_bf16:
                    assert self.dtype == th.bfloat16
                else:
                    assert self.dtype == th.float16
            h = module(h, emb, use_fp16_or_bf16=self.use_fp16_or_bf16, dtype=self.dtype)
            # assert not th.isnan(h).any(), f'timestep: {timestep}'
            hs.append(h)
        h = self.middle_block(h, emb)
        # assert not th.isnan(h).any(), f'timestep: {timestep}'
        for module in self.output_blocks:
